...
function SupermemoryContent({
    ...
    useEffect(() => {
        const isTwitter = window.location.hostname.match(
            /^(twitter\.com|x\.com)$/,
        )
        if (isTwitter) {
            chrome.storage.local.get(
                ["attemptingImportCurrently"],
                (result) => {
                ...
                switch (message.payload.status) {
                    case 429:
                        setImportStatus({
                            status: 429,
                            message:
                                "Rate limited by Twitter. Waiting to retry...",
                        })
                        break
                    case 409:
                        setImportStatus({
    ...
    const savePage = async () => {
        ...
        try {
            const response = await sendMessage("savePage", {
                        ...
                        <div className="flex flex-col gap-0.5">
                            ...
                            {importComplete && (
                                <span className="text-lg font-semibold text-green-400">
                                    Successfully imported {importProgress}{" "}
                                    tweets
                                </span>
                            )}
                        </div>
