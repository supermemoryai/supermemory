        ...
        if (response.status === 400 && data.errors?.[0]?.message) {
            ...
            if (missingFeatures) {
                ...
                featuresList.forEach((feature: string) => {
                    updatedFeatures[feature as keyof typeof updatedFeatures] =
                        true
                })
    ...
    chrome.webRequest.onBeforeSendHeaders.addListener(
        (details) => {
            try {
                            ...
                            if (match?.[1] && !result.bookmarksApiId) {
                                ...
                                chrome.storage.local.set(
                                    ...
                                    () => {
                                        ...
                                        console.log(
                                            `Stored bookmarksApiId: ${bookmarksApiId}`,
                                        )
                                    },
                                )
                            }
                ...
                console.error(
                    "Top level error in onBeforeSendHeaders listener:",
                    err,
                )
            }
        },
