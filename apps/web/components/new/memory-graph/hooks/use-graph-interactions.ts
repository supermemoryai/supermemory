...
export function useGraphInteractions(
	...
	const zoomOut = useCallback(
		(centerX?: number, centerY?: number, animate = true) => {
			...
			const newZoom = Math.max(0.05, zoom * zoomFactor) // Decreased min zoom to 0.05
	...
	const resetView = useCallback(
		...
		[settings, isAnimating, animateToViewState],
	)
	...
	const autoFitToViewport = useCallback(
		(
			...
			// Account for occluded area (e.g., chat panel on the right)
			const occludedRightPx = options?.occludedRightPx ?? 0
			const availableWidth = viewportWidth - occludedRightPx

			// Calculate zoom to fit content
			const zoomX = availableWidth / contentWidth
			const zoomY = viewportHeight / contentHeight
			const newZoom = Math.min(zoomX, zoomY, 1) // Cap at 1x to avoid over-zooming
