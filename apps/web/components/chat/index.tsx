...
export function ChatSidebar({
	...
	const [selectedModel, setSelectedModel] = useState<ModelId>(
		() => initialSelectedModel ?? "gemini-2.5-pro",
	)
		...
		<motion.div
			...
			className={cn(
				...
				isMobile
					...
					: isPageDesktop
						? "h-full min-h-0 w-full min-w-0 flex-1 basis-0 rounded-none border-x-0"
		...
		>
			...
			{isPageDesktop ? (
				<>
					...
					<div className="mx-auto flex h-full min-h-0 w-full max-w-[720px] min-w-0 flex-1 flex-col px-4">
						{shell}
					</div>
				</>
			) : (
