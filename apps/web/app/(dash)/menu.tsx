	...
	return (
		<>
			...
			<Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
				...
				<DialogContent className="sm:max-w-[600px] text-[#F2F3F5] rounded-2xl bg-background z-[39]">
					<Content
						setDialogOpen={(t) => {
							setDialogOpen(t);
						}}
...
function Content({ setDialogOpen }: { setDialogOpen: (t: boolean) => void }) {
	...
	return (
		...
		>
			<DialogHeader>
				...
				<Label htmlFor="space">Save to a space (Optional)</Label>
				<div className="flex">
						...
						<div className="flex flex-row flex-wrap gap-0.5 h-min">
							{[...new Set(selectedSpaces)].map((x, idx) => (
								<button
									...
									className={`relative group px-2 py-1 bg-[#3C464D] max-w-32 rounded-lg`}
								>
