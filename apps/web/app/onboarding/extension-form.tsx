...
type CursorAction =
    ...
    | { type: 'move'; target: React.RefObject<HTMLElement | null>; duration: number }
...
function Cursor({ actions, className, onPositionChange }: CursorProps) {
    ...
    function moveToElement(elementRef: React.RefObject<HTMLElement | null>, duration: number) {
    ...
    return (
        <div ref={containerRef} className={`absolute inset-0 pointer-events-none ${className || ''}`}>
            ...
            >
                <MousePointer2 className="size-6 drop-shadow" strokeWidth={1.5} fill="white" />
            </motion.div>
        </div>
    )
}
...
function SnippetDemo() {
    ...
    return (
        <div ref={snippetRootRef} className="size-full select-none text-xs relative overflow-hidden">
            <Cursor
                actions={cursorActions}
                onPositionChange={function onPositionChange(clientX: number, clientY: number) {
                    // Handle text highlighting
                    const textIdx = getIndexFromClientPoint(clientX, clientY);
                    const next = textIdx < lastStableIndexRef.current ? lastStableIndexRef.current : textIdx;
                        ...
                        <div className="flex flex-col w-48 text-left absolute top-0 right-0 text-white bg-zinc-800/70 text-xs border-0.5 backdrop-blur-sm border-zinc-900/80 rounded-md px-1.5 py-1.5">
                            {["Back", "Forward", "Reload", "Save As...", "Print...", "Translate to English", "Save to Supermemory", "View Page Source", "Inspect"].map((item, idx) => (
                                <React.Fragment key={item}>
                                    <div
                                        ref={el => { menuItemRefs.current[idx] = el; if (idx === 6) { menuItem6Ref.current = el; } }}
                                        data-menu-idx={idx}
                                        className={cn(
                                    ...
                                    >
                                        {idx === 6 && <img src="/images/icon-16.png" alt="Supermemory" className="size-3.5" />}
                                        {item}
                                    </div>
                                    {[2, 5, 6].includes(idx) && <div className="h-px rounded-full my-1 mx-2 bg-zinc-300/20" />}
                                </React.Fragment>
                            ))}
                        </div>
                    )}
...
function ChatGPTDemo() {
    ...
    return (
        <div className="size-full relative overflow-hidden select-none pointer-events-none text-white flex flex-col gap-6 items-center justify-center" style={{
            ...
            <div className="w-[85%] text-sm rounded-3xl p-2 flex flex-col gap-2" style={{ backgroundColor: "#303030" }}>
                <div className="p-2">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="h-4.5 flex items-center">
                            <motion.div
                                ...
                                initial={false}
                                animate={{
                                    backgroundColor: enhancementStatus === "notStarted" ? "transparent" : "#1e1b4b",
                                    borderColor: enhancementStatus === "notStarted" ? "transparent" : "#4338ca",
                                    borderWidth: enhancementStatus === "notStarted" ? 0 : 1,
                                    paddingLeft: enhancementStatus === "notStarted" ? 0 : (enhancementStatus === "enhancing" ? 4 : 8),
                                    paddingRight: enhancementStatus === "notStarted" ? 0 : (enhancementStatus === "enhancing" ? 6 : 8),
                            ...
                            >
                                {enhancementStatus === "notStarted" && (
                                    <img src="/images/icon-16.png" alt="Enhance with Supermemory" className="size-5" />
                                )}
                                ...
                                {enhancementStatus === "done" && (
                                    <>
                                        ...
                                        <motion.span
                                            ...
                                            animate={{ opacity: 1, width: "auto" }}
                                            transition={{ delay: 0.1, duration: 0.2 }}
                                            className="ml-2">Including 1 memory</motion.span>
                                    </>
                                )}
                                <AnimatePresence>
                                    {memoriesExpanded && (
                                        ...
                                        >
                                            <div className="flex items-center gap-2">
                                                <img src="/images/icon-16.png" alt="Enhance with Supermemory" className="size-5" />
                                                <span className="text-xs">User possesses an American Express Platinum card</span>
                                            </div>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                        ...
                        <div ref={submitButtonRef} className="rounded-full bg-white size-6 flex items-center justify-center">
            ...
            <div className="bg-white text-black px-5 py-3 text-sm rounded-2xl w-9/10">
                <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-base font-medium">Import Twitter Bookmarks</span>
                    </div>
                ...
                <div className="mt-3">
                    <motion.button
                        ref={importButtonRef}
                        animate={{
                            backgroundColor: importStatus === "importing"
                                ? "#f59e0b"
                                : importStatus === "done"
                                    ? "#10b981"
                                    : "#3b82f6"
