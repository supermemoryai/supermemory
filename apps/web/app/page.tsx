...
interface DocumentWithCustomId extends DocumentWithMemories {
	customId?: string;
}
...
const MemoryGraphPage = () => {
	...
	const isCurrentProjectExperimental = !!projectsMeta.find(
		(p: ProjectMeta) => p.containerTag === selectedProject,
	)?.isExperimental;
	...
	useEffect(() => {
		...
		for (const d of [...baseDocuments, ...injectedDocs]) {
			...
			const docWithCustomId = d as DocumentWithCustomId;
			if (docWithCustomId.customId) present.add(docWithCustomId.customId);
		}
		...
		const run = async () => {
			try {
				...
				const apiResp = resp as unknown as ApiResponse;
				if (cancelled || apiResp?.error) return;
				const extraDocs = apiResp?.data?.documents;
...
export default function Page() {
	...
	useEffect(() => {
		// Get the raw token from URL without URL decoding
		const url = new URL(window.location.href);
		const rawToken = url.searchParams.get("token");

		if (rawToken) {
			// Re-encode the token to preserve the original encoding
			const encodedToken = encodeURIComponent(rawToken);
			console.log("Token extracted:", encodedToken);
