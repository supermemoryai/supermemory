...
function Reminder({
    ...
    return (
        ...
        >
            ...
            >
                <div
                    className={`${!isExpanded && "absolute inset-0"} overflow-hidden`}
                >
...
function Reminders() {
    ...
    if (
        !isLoading &&
        (!suggestedLearnings || suggestedLearnings.length === 0)
    ) {
        return (
                ...
                <div className="text-center max-w-lg mx-auto px-4">
                    ...
                    >
                        ...
                        <p className="text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
                            Start saving content to get bite-sized memory
                            snippets that keep you on top of your game. We'll
                            transform your notes into smart reminders that help
                            you retain knowledge effortlessly.
                        </p>
                        <AddMemoryModal>
        ...
        >
            <div className="relative w-full h-full translate-y-16">
                {isLoading
                    ? // Show 3 loading cards when loading
                      [...Array(3)]?.map((_, index) => {
                          const xOffset =
                              index * (window?.innerWidth < 768 ? 12 : 16)
                          const yOffset =
                              index * (window?.innerWidth < 768 ? 12 : 16)
                          const opacity = Math.max(1 - index * 0.15, 0.4)
                          const scale =
                              1 -
                              index * (window?.innerWidth < 768 ? 0.02 : 0.03)
                    ...
                    : suggestedLearnings?.map((learning, index) => {
                          ...
                          const xOffset =
                              position * (window?.innerWidth < 768 ? 12 : 16)
                          const yOffset =
                              position * (window?.innerWidth < 768 ? 12 : 16)
                          ...
                          const scale =
                              1 -
                              position *
                                  (window?.innerWidth < 768 ? 0.02 : 0.03)
                          ...
                          return (
                              <motion.div
                                  ...
                                  onDragEnd={
                                      position === 0 ? handleDragEnd : undefined
                                  }
                                  whileHover={
                                      position === 0 &&
            ...
            {!isLoading && suggestedLearnings.length > 0 && (
                <div className="md:hidden absolute bottom-[-24px] left-1/2 transform -translate-x-1/2 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                    Swipe to navigate • {currentIndex + 1}/
                    {suggestedLearnings.length}
                </div>
            )}
        </div>
