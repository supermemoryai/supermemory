        ...
        async (file: File) => {
            ...
            try {
                ...
                if (error) {
                    ...
                    setPreviews((prev) =>
                        prev.filter((_, i) => i !== fileURLs.length),
                    )
                    return
                }
                if (fileURL) {
        ...
        >
            <div
                className={cn(
                    ...
                    mini &&
                        "flex flex-col md:flex-row items-center rounded-2xl",
                )}
            >
                <input
                    type="file"
            ...
            {previews.length > 0 && (
                <div className="flex flex-wrap gap-2 p-4 border-t border-gray-200">
                    {fileURLs.map((fileURL, index) => {
                        const isPDF = previews[index].startsWith(
                    ...
                    {fileURLs.length !== previews.length &&
                        previews[fileURLs.length] && (
                            <>
                                {previews[fileURLs.length].startsWith(
                                    "data:application/pdf",
                                ) ? (
