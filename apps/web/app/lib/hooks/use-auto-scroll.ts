...
export function useAutoScroll({
    ...
    const scrollToBottom = (behavior: ScrollBehavior = "smooth") => {
        if (bottomRef.current && shouldAutoScroll) {
            ...
            const targetScrollTop =
                bottomRef.current.offsetTop - parent.clientHeight + offset
    ...
    const handleScroll = () => {
        ...
        const targetScrollTop =
            bottomRef.current.offsetTop - clientHeight + offset

        // Consider the user at the bottom if they're within the threshold of the target scroll position
        const isAtBottom =
            Math.abs(targetScrollTop - scrollTop) < bottomThreshold
