...
describe("collectValidUrls", () => {
	...
	it("filters Twitter links with uppercase URL schemes", () => {
		expect(collectValidUrls("", ["HTTP://x.com/supermemoryai"])).toEqual([])
		expect(collectValidUrls("", ["HTTPS://twitter.com/supermemoryai"])).toEqual(
			[],
		)
	})
