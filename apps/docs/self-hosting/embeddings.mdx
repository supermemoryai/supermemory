...
|---|---|---|
| `SUPERMEMORY_EMBEDDING_PROVIDER` | Embedding backend: `local`, `openai`, `openai-compatible`, or `gemini` (for Ollama, use `openai-compatible` with `SUPERMEMORY_EMBEDDING_BASE_URL`) | `local` |
...
SUPERMEMORY_EMBEDDING_API_KEY=ollama
