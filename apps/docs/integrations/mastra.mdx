...
<Note>
  **Memory saving is enabled by default.** The wrapper automatically saves conversations. To disable saving (read-only mode):
...
| `mode` | `"profile" \| "query" \| "full"` | `"profile"` | Memory retrieval mode |
| `addMemory` | `"always" \| "never"` | `"always"` | Auto-save conversations |
| `searchMode` | `"memories" \| "hybrid" \| "documents"` | `"memories"` | Search endpoint mode |
| `searchLimit` | `number` | `10` | Max search results per endpoint |
...
| Search Mode | Endpoint | Use Case |
|-------------|----------|----------|
| `memories` | `/v4/search` | Conversational memory, low latency |
| `documents` | `/v3/search` | RAG with uploaded docs |
| `hybrid` | Both | Comprehensive context |
...
interface SupermemoryMastraOptions {
  ...
  searchMode?: "memories" | "hybrid" | "documents"  // Search endpoint mode
