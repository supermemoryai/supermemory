...
export async function GET() {
  ...
  // Fallback for servers without the rich graph endpoint (e.g. self-hosted
  // supermemory-server returns 404 for /documents/documents).
  ...
  // Normalize /list ({ memories }) to graph shape ({ documents }).
  if (!data.documents && Array.isArray(data.memories)) {
    return NextResponse.json({
      documents: data.memories.map((m) => ({ ...m, memories: [], memoryEntries: [] })),
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const body = JSON.stringify({ page: 1, limit: 500, sort: 'createdAt', order: 'desc' });
  const headers = {
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${process.env.SUPERMEMORY_API_KEY}`,
  };

  let response = await fetch('https://api.supermemory.ai/v3/documents/list', {
    method: 'POST',
    headers,
    body,
  });
