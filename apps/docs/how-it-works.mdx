...
Supermemory is built for change—the graph keeps every version of a memory, recording when it became true and when it expired. The web console now exposes an optional **temporal view** (behind the `NEXT_PUBLIC_ENABLE_TEMPORAL_QUERIES` flag):
