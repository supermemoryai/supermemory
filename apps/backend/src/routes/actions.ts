...
import { type Env, type Variables, recommendedQuestionsSchema } from "../types"
...
const actions = fromHono(new Hono<{ Variables: Variables; Bindings: Env }>())
    .post(
        ...
        async (c) => {
            ...
            const lastUserMessage = coreMessages.findLast(
                (i) => i.role === "user",
            )
            const queryText =
            ...
            try {
                ...
                for (const chunk of contextChunks) {
                    const docChunks =
                        chunksByDocument.get(chunk.documentId) || []
                ...
                const contextualResults = matchingChunks.map((match) => {
                    const docChunks =
                        chunksByDocument.get(match.documentId) || []
                    const matchIndex = docChunks.findIndex(
                        (c) => c.id === match.chunkId,
                    )
  ...
  The user's saved content is provided in <context> tags. Use this information naturally without explicitly referencing it.`,
    ...
    .get(
        ...
        async (c) => {
            ...
            const chatHistory = await database(
                ...
                .where(
                    and(
                        ...
                        eq(chatThreads.uuid, threadUuid),
                    ),
                )
  ...
  ${recentDocuments.map((d) => d.content).join("\n\n")}`,
    ...
    .get("/suggested-learnings", async (c) => {
        ...
        if (cached) {
            return c.json({
                suggestedLearnings: JSON.parse(cached) as {
                    [x: string]: string
                },
            })
        }
    ...
    .post(
        ...
        async (c) => {
            ...
            if (spaces && spaces.length > 0) {
                ...
                const unauthorized = validSpaces.filter(
                    (s) => s.ownerId !== user.id,
                )
            ...
            try {
                ...
                for (const result of results) {
                    const existingResult = documentResults.get(
                        result.documentId,
                    )
                    if (
                        !existingResult ||
                ...
                const spacePermissions = await Promise.all(
                    body.spaces.map(async (spaceId) => {
                        ...
                        return {
                            ...
                            allowed:
                                spaceData.ownerId === user.id ||
                                spaceAccessCheck.length > 0,
                            error:
                                spaceData.ownerId !== user.id &&
                                !spaceAccessCheck.length
                ...
                async start(controller) {
                    ...
                    for (const item of items) {
                        try {
                            ...
                            const title =
                                typeof item === "string" ? null : item.title
                            const type =
                                typeof item === "string"
                                    ? typeDecider(item)
                                    : Ok(item.type)
